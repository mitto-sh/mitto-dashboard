...
import type { GithubInstallation, GithubRepo, Project } from '@/lib/types'
...
const project: Project = { id: 'p1', name: 'api', slug: 'api', repoUrl: repo.html_url, runtime: null, region: 'us-east-1' }
...
describe('ImportFromGithubModal', () => {
  it('shows a Connect GitHub button when there are no installations', async () => {
    ...
    vi.mocked(api.githubInstallUrl).mockResolvedValue({ url: 'https://github.com/apps/mitto-sh-dev/installations/new?state=x' })
  ...
  it('lists installations, then repos, then confirms an import with detected services', async () => {
    ...
    vi.mocked(api.createService).mockResolvedValue({ id: 's1', projectId: 'p1', name: 'web', type: 'web', port: 3000, cpu: 256, memory: 512 })
    ...
    await waitFor(() => {
      expect(api.createProject).toHaveBeenCalledWith({ name: 'api', repoUrl: repo.html_url })
      expect(api.createService).toHaveBeenCalledWith({ projectId: 'p1', name: 'web', type: 'web', port: 3000 })
      expect(onImported).toHaveBeenCalledWith(project)
    })
  })

  it('offers to import without services when no mitto.yaml is found', async () => {
    vi.mocked(api.listGithubInstallations).mockResolvedValue([installation])
    vi.mocked(api.listInstallationRepos).mockResolvedValue([repo])
    ...
    expect(await screen.findByText(/No mitto.yaml found/)).toBeInTheDocument()
